...
from sqlalchemy import CheckConstraint, Column, ForeignKeyConstraint, Index, Integer, String, UniqueConstraint
...
class LegacyMemberRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("record_kind = 'ARCHIVED_LEGACY_MEMBER'", name="ck_legacy_member_records_kind"),
        CheckConstraint("reason_code = 'UNLINKED_HISTORICAL_PAYMENT'", name="ck_legacy_member_records_reason"),
        ForeignKeyConstraint(["gym_id"], ["gyms.id"], name="fk_legacy_member_records_gym_id_gyms", ondelete="RESTRICT"),
        ForeignKeyConstraint(["gym_id", "created_by_user_id"], ["users.gym_id", "users.id"], name="fk_legacy_member_records_gym_user", ondelete="RESTRICT"),
        UniqueConstraint("gym_id", "id", name="uq_legacy_member_records_gym_id_id"),
        UniqueConstraint("gym_id", "source_reference", name="uq_legacy_member_records_gym_source"),
    ...
    reason_code = Column(String(64), nullable=False, default="UNLINKED_HISTORICAL_PAYMENT")
