...
def register_member(client, registration_date: date | None = None):
    response = client.post(
        ...
        data={
            ...
            "registration_date": str(registration_date or date.today()),
        },
        follow_redirects=False,
    )
    assert response.status_code == 303
...
def test_single_month_renewal_creates_payment_and_extends_from_current_due_date(client, db):
    ...
    assert payments[-1].payment_date == date.today()
...
def test_invalid_renewal_amount_does_not_change_membership_or_create_payment(client, db, amount):
    ...
    response = client.post(
        f"/members/{member.id}/renew", data={"amount": amount})
...
def test_expired_member_renewal_restarts_from_today(client, db):
    configure_gym(client)
    register_member(client, date.today() - relativedelta(months=3))
    member = get_member(db)
    assert member.payment_due_date < date.today()
    ...
    db.refresh(member)
    assert response.status_code == 303
    assert member.payment_due_date == date.today() + relativedelta(months=2)
    assert member.status == "Active"


def test_status_calculation_marks_due_today_active_and_past_due_expired(db):
    gym = Gym(name="Status Test Gym", currency="GHS",
              registration_fee=200, monthly_fee=120)
    db.add(gym)
    db.flush()
...
def test_member_detail_shows_payment_history(client, db):
    ...
    register_member(client)
    member = get_member(db)
    client.post(f"/members/{member.id}/renew", data={"amount": MONTHLY_FEE})
...
def test_member_deletion_removes_member_and_preserves_orphaned_payment_history(client, db):
    configure_gym(client)
    register_member(client)
    member = get_member(db)
    client.post(f"/members/{member.id}/renew", data={"amount": MONTHLY_FEE})
    ...
    response = client.post(
        f"/members/{member.id}/delete", follow_redirects=False)
    ...
    assert db.query(Member).count() == 0
    assert len(payments) == 2
    assert all(payment.member_id is None for payment in payments)
...
def test_dashboard_calculates_membership_and_revenue_totals(client, db):
    ...
    register_member(client)
    member = get_member(db)
    client.post(f"/members/{member.id}/renew",
                data={"amount": MONTHLY_FEE * 2})
