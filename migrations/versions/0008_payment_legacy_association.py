...
def _payment_9_for_repair(bind: sa.Connection) -> dict[str, object] | None:
    row = (
        bind.execute(
            sa.text("""
    ...
    # A brand-new database has no pre-existing historical payment 9.  This
    # branch is necessary for a complete fresh migration chain; it never
    # changes an existing release database, where payment 9 must be present
    # and match the approved facts exactly.
    if row is None:
        return None
    if any(row[key] != value for key, value in PAYMENT_9_FACTS.items()):
...
def _assert_association_violations(
    bind: sa.Connection, expected_ids: list[int]
) -> None:
    violating_ids = (
        bind.execute(
            sa.text("""
    ...
    if violating_ids != expected_ids:
        raise RuntimeError(
            "Unexpected pre-existing payment association invariant violation."
        )
...
def _legacy_record_id(bind: sa.Connection, gym_id: int) -> int:
    rows = (
        ...
        FOR UPDATE
    """),
            {"gym_id": gym_id, "source_reference": LEGACY_SOURCE_REFERENCE},
        )
        .mappings()
        .all()
    )
    if not rows:
        ...
        """),
            {
                "gym_id": gym_id,
                "record_kind": LEGACY_RECORD_KIND,
    ...
    record = rows[0]
    if (
        record["record_kind"] != LEGACY_RECORD_KIND
...
def _assert_repair(bind: sa.Connection, gym_id: int, legacy_record_id: int) -> None:
    ...
    expected = {
        "member_id": None,
        "legacy_member_record_id": legacy_record_id,
        "gym_id": gym_id,
        "record_kind": LEGACY_RECORD_KIND,
        "reason_code": LEGACY_REASON_CODE,
        "source_reference": LEGACY_SOURCE_REFERENCE,
    }
    if repaired is None or any(
        repaired[key] != value for key, value in expected.items()
    ):
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_legacy_member_records_gym_id", "legacy_member_records", ["gym_id"]
    )
    op.add_column(
        "payments", sa.Column("legacy_member_record_id", sa.Integer(), nullable=True)
    )
    op.create_index(
        "ix_payments_legacy_member_record_id", "payments", ["legacy_member_record_id"]
    )
    ...
    payment = _payment_9_for_repair(bind)
    _assert_association_violations(bind, [9] if payment is not None else [])
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_payments_gym_legacy_member_record", "payments", type_="foreignkey"
    )
